...
# Check if a specific language was provided as argument
if [ -n "$1" ]; then
	languages=("$1")
	printf "Processing only language: %s\n" "$1"
fi
...
for lang in "${languages[@]}" ; do
	if [ -f $lang.po ]; then \
		...
		msgmerge --backup=none --no-wrap --no-location --no-fuzzy-matching -s -U $lang.po enigma2.pot && touch $lang.po; \
		msgattrib --no-wrap --no-obsolete --no-fuzzy $lang.po -o $lang.po; \
