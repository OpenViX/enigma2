...
// iDVBChannelList
	...
	RESULT addOrUpdateBouquet(const std::string &name, const std::string &filename, SWIG_PYOBJECT(ePyObject) services, const int type, bool isAddedFirst = false);
...
//////
	...
	void deleteBouquet(const std::string filename, int type);
