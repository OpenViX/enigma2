...
class PiconLocator:
	...
	def getPiconName(self, serviceRef):
		...
		if not pngname:  # picon by channel name
			if (sname := eServiceReference(serviceRef).getServiceName().replace('\x86', '').replace('\x87', '')) and "SID 0x" not in sname and (utf8_name := sanitizeFilename(sname).lower()) and utf8_name != "__":  # avoid lookups on zero length service names
