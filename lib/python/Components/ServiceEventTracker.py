...
class ServiceEventTracker:
	...
	def event(evt):
		...
		if func_list:
			...
			old_service_running = set.oldRef and cur_ref and cur_ref == set.oldRef and hasattr(nav.getCurrentService(), "getPtrString") and set.oldServiceStr == nav.getCurrentService().getPtrString()
