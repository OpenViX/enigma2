...
class NimManager:
	...
	def enumerateNIMs(self):
		...
		for id, entry in entries.items():
			...
			if entry["name"] and ("fbc" in entry["name"].lower() or "45308X" in entry["name"].upper() or (entry["name"] in SystemInfo["HasFBCtuner"] and entry["frontend_device"] is not None and access("/proc/stb/frontend/%d/fbc_id" % entry["frontend_device"], F_OK))):
				fbc_number += 1
